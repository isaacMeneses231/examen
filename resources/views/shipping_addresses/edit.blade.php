                ...
                <form action="{{ route('shipping_addresses.update', $shipping_address) }}" method="POST" id="form-edit-{{ $shipping_address->id }}" onsubmit="confirmarActualizacion(event, {{ $shipping_address->id }})" novalidate>
                    ...
                    <div class="mb-6">
                        ...
                        <select id="client_id" name="client_id" class="w-full rounded-xl border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring focus:ring-indigo-500 focus:ring-opacity-20 shadow-sm transition-colors">
                            ...
                            @foreach($clients as $client)
                                <option value="{{ $client->id }}" {{ old('client_id', $shipping_address->client_id) == $client->id ? 'selected' : '' }}>
                                    {{ $client->full_name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-6">
                        <div>
                            ...
                            <input type="text" id="city" name="city" value="{{ old('city', $shipping_address->city) }}" maxlength="255" class="w-full rounded-xl border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring focus:ring-indigo-500 focus:ring-opacity-20 shadow-sm transition-colors">
                            @error('city')
                                <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                        <div>
                            ...
                            <input type="text" id="neighborhood" name="neighborhood" value="{{ old('neighborhood', $shipping_address->neighborhood) }}" maxlength="255" class="w-full rounded-xl border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring focus:ring-indigo-500 focus:ring-opacity-20 shadow-sm transition-colors">
                            @error('neighborhood')
                                <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="grid grid-cols-4 gap-4 mb-6">
                        <div class="col-span-3">
                            ...
                            <input type="text" id="street" name="street" value="{{ old('street', $shipping_address->street) }}" maxlength="255" class="w-full rounded-xl border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring focus:ring-indigo-500 focus:ring-opacity-20 shadow-sm transition-colors">
                            @error('street')
                                <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" id="address_status" name="address_status" value="{{ old('address_status', $shipping_address->address_status) }}" maxlength="255" class="w-full rounded-xl border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring focus:ring-indigo-500 focus:ring-opacity-20 shadow-sm transition-colors">
    ...
    <script>
        function confirmarActualizacion(event, id) {
            ...
            Swal.fire({
                ...
                text: "Se actualizará la información de esta dirección.",
                icon: 'question',
