                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-gray-50/50 dark:bg-gray-900/50 border-b border-gray-100 dark:border-gray-700">
                                ...
                                <th class="px-6 py-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Código</th>
                                <th class="px-6 py-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Descripción</th>
                                <th class="px-6 py-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Precios</th>
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                            @forelse ($articles as $article)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors group">
                                    <td class="px-6 py-4">
                                        <span class="text-sm font-mono text-gray-400 dark:text-gray-500">{{ $article->id }}</span>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="text-sm font-mono font-semibold text-indigo-600 dark:text-indigo-400">
                                    ...
                                    <td class="px-6 py-4">
                                        ...
                                        <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                                            Ingreso: {{ \Carbon\Carbon::parse($article->entry_date)->format('d/m/Y') }}
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="text-sm text-gray-600 dark:text-gray-400">
                                            <span class="font-semibold text-gray-800 dark:text-gray-200">Venta:</span> ${{ number_format($article->sale_price, 2) }} <br>
                                            <span class="text-xs">Costo: ${{ number_format($article->purchase_cost, 2) }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        <div class="flex justify-end items-center space-x-3 opacity-0 group-hover:opacity-100 transition-opacity">
                                            ...
                                            <a href="{{ route('articles.edit', $article) }}" class="text-gray-400 hover:text-amber-500 transition-colors" title="Editar">
                                            ...
                                            <form action="{{ route('articles.destroy', $article) }}" method="POST" class="inline" id="form-delete-{{ $article->id }}">
                                                @csrf
                                                @method('DELETE')
                                                <button type="button" onclick="confirmarEliminacion({{ $article->id }})" class="text-gray-400 hover:text-red-500 transition-colors" title="Eliminar">
