        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-2xl border border-gray-100 dark:border-gray-700 p-8">
                ...
                <div class="mb-6 grid grid-cols-2 gap-6">
                    <div>
                        <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">Cliente Asociado</h3>
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400">
                            {{ $order->client->full_name ?? 'No asignado' }}
                        </span>
                    </div>
                    <div>
                ...
                <div class="mt-8 border-t border-gray-100 dark:border-gray-700 pt-6 grid grid-cols-2 gap-6">
                    <div>
                        <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">Fecha del Pedido</h3>
                        <p class="text-md text-gray-900 dark:text-white">{{ \Carbon\Carbon::parse($order->order_date)->format('d/m/Y h:i A') }}</p>
                    </div>
                    <div>
                        <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">ID Dirección de Envío</h3>
                        <p class="text-md text-gray-900 dark:text-white">Dirección #{{ $order->shipping_address_id }}</p>
                    </div>
                </div>
                ...
                <div class="mt-8 border-t border-gray-100 dark:border-gray-700 pt-6 grid grid-cols-3 gap-6 bg-gray-50 dark:bg-gray-900/50 p-4 rounded-xl">
                    <div>
                        <h3 class="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Subtotal</h3>
                        <p class="text-lg font-semibold text-gray-900 dark:text-white">${{ number_format($order->subtotal, 2) }}</p>
                    </div>
                    <div>
                        <h3 class="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Monto IVA</h3>
                        <p class="text-lg font-semibold text-gray-900 dark:text-white">${{ number_format($order->iva_amount, 2) }}</p>
                    </div>
                    <div>
                        <h3 class="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Total General</h3>
                        <p class="text-xl font-bold text-green-600 dark:text-green-400">${{ number_format($order->general_total, 2) }}</p>
                    </div>
                </div>
                ...
                <div class="mt-8 border-t border-gray-100 dark:border-gray-700 pt-6">
                    ...
                    <div class="text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-900/50 p-4 rounded-xl border border-gray-100 dark:border-gray-700 min-h-[100px] whitespace-pre-line">
                        {{ $order->additional_notes ?? 'No hay notas adicionales registradas para este pedido.' }}
                    </div>
                </div>
